...
**외부에 노출되면 안되는 중요한 비밀번호**
    ...
    무조건 영어 대분자로 사용 띄어쓰기 대신에 _를 사용

    **vite 환경에서는 무조건 환경변수 앞에 VITE_를 붙혀서 사용**

파이어스토어, 파이어스토어서비스, 인증, 인증서비스, 파이어베이스 그자체

```javascript
const db = firebase.firestore; //! 파이어스토어 그자체
const dbService = firebase.firestore(); //! firestore의 database기능

const auth = firebase.auth; //! 인증 그자체
const authService = firebase.auth(); //! 인증서비스 기능

export { firebase, db, dbService, auth, authService };
```

2. React에서 firebase CRUD하기

   1. ref 하는 어떤 컬렉션을 사용하는지 정의해둔 변수 사용

   ```javascript
   const ref = dbSrvice.collection("컬랙션이름");
   ```
   ...
   1. useState, useEffect사용
      ...
      가져올 데이터를 담을 컨테이너를 useState로 만듬
 ...
 const [items, setItems] = useState<Item[]>([])
  ...
  useEffect(
      ()=>{
          ...
          //! eslint 에러가 발생하면 해당 기능을 off로 변경하기
          subScribItem
...
//! 비동기 함수로 코드 안짜면 데이터베이스 안기다리고 코드가 막 실행됨
```

3. CREATE 데이터베이스에 자료 추가하기

ref.add(데이터) //! 아이디를 자동 생성
...
ref.doc(아이디).update(수정할 내용 또는 새로운 추가할 내용)
